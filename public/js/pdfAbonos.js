    ...
    fetch(urlAbonos, {
      ...
      .then(dataAbonos => {
        ...
        fetch(urlVentas, {
         ...
         // Mapear los abonos y excluir los campos _id y __v
                const abonosConNumeroFactura = abonos.map(abono => {
                    const { _id, __v, ...abonoSinIdV } = abono;
                    const ventaEncontrada = ventas.find(venta => venta._id === abono.venta);
                    const numeroFactura = ventaEncontrada ? ventaEncontrada.numeroFactura : "";
